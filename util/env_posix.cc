...
class PosixRandomAccessFile final : public RandomAccessFile {
   public:
    ...
    Status Read(uint64_t offset, size_t n, Slice* result,
        ...
        Status status;
        size_t read_size = ::pread(fd, scratch, n, static_cast<off_t>(offset));
...
class PosixMmapWriteableFile final : public WritableFile {
   public:
    PosixMmapWriteableFile(const std::string& fname, int fd, size_t page_size)
        : filename_(fname),
          fd_(fd),
    ...
    ~PosixMmapWriteableFile() override {
        if (fd_ >= 0) {
   ...
   private:
    ...
    Status MapNewRegion() {
        ...
        if (is_pmem != 1) {
            printf("ssd not pmem\n");
            return Status::IOError("ssd,not pmem!\n");
        }
